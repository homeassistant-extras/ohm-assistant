...
describe('feature', () => {
  it('should return true when config is null', () => {
    expect(hasFeature(null as any as Config, 'hide_name')).to.be.true;
  });

  it('should return true when config is undefined', () => {
    expect(hasFeature(undefined as any as Config, 'hide_name')).to.be.true;
  });

  it('should return false when config.features is undefined', () => {
    const config = {} as Config;
    expect(hasFeature(config, 'hide_name')).to.be.false;
  });
  ...
  it('should return true when feature is present in config.features', () => {
    const config = {
      area: '',
      features: ['hide_name', 'exclude_default_entities'],
    } as Config;
    expect(hasFeature(config, 'hide_name')).to.be.true;
  });
